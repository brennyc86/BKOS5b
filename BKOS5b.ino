...
// Bluetooth UUIDs
#define SERVICE_UUID "4aaf20c2-1b3a-4f8e-8c5a-9e1234567890"
#define CHARACTERISTIC_UUID "beb5483e-36e1-4688-b7f5-a1234567890"

BLECharacteristic *pCharacteristic;
BLEServer *pServer;

void setupBluetooth() {
  BLEDevice::init("BKOS5b Board Computer");
  ...
  pCharacteristic->setValue("BKOS5b Ready");
  pService->start();
...
void handleBluetoothCommand(String command) {
  if (command == "status") {
    pCharacteristic->setValue("System operational");
    pCharacteristic->notify();
  } else if (command.startsWith("set_port:")) {
    // Parse port command
    int port = command.substring(9).toInt();
    // TODO: Set port state
    pCharacteristic->setValue("Port " + String(port) + " updated");
    pCharacteristic->notify();
  }
}

// ========== WiFi API Functionaliteit ==========
...
void setupWiFiAPI() {
  ...
  webServer.on("/api/status", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "application/json", "{\"status\": \"ok\", \"version\": \"BKOS5b\"}");
  });
  
  webServer.on("/api/ports", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "application/json", "{\"ports\": [{\"id\": 1, \"name\": \"Light\", \"state\": true}]}");
  });
  
  webServer.on("/api/signal", HTTP_POST, [](AsyncWebServerRequest *request) {
    // TODO: Signal/WhatsApp integration
    request->send(200, "text/plain", "Message sent via Signal");
  });
  
  webServer.begin();
  Serial.println("WiFi API started on port 8080");
}
